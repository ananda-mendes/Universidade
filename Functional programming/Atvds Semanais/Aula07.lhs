...
evenList :: [Int] -> [Bool]
evenList [] = []
evenList xs = [if (even x) then True else False | x <- xs]
...
- Definir uma função que, a partir de uma string,
...
estaNoIntervalo a b x 
  | x >= a && x <= b = True
  | otherwise = False

intervaloFilter :: [Int] -> (Int, Int) -> [Int]
intervaloFilter list (a,b) = filter (estaNoIntervalo a b) list

---RECURSIVO
recursive :: [Int] -> (Int,Int) -> [Int]
recursive [] (_, _) = []
recursive (x:xs) (a, b) 
  | x >= a && x <= b = x : recursive xs (a,b) 
  | otherwise = recursive xs (a,b)
\end{code}
...
\begin{code}
sumOdd :: [Int] -> Int
sumOdd = sum . map (\x -> x^2) . filter odd
\end{code}
...
takeWhileFR f xs = foldr step [] xs
  where
    step x ac = if (f x) then (x:ac) else []
...
allR :: (a -> Bool) -> [a] -> Bool
allR f [] = True
allR f list = allR' f list True
  where
    allR' f [] b = b
    allR' f [x] b = allR' f [] ((f x) && b)
    allR' f (x:xs) b = allR' f xs ((f x) && b)
allFR :: (a -> Bool) -> [a] -> Bool
allFR f xs = foldr step True xs
  where
    step x xs = (f x) && xs
