...
indexOf x (y : xs) 
     | (x == y) = 1
     | (x >= y || x <= y) && elem x xs = 1 + indexOf x xs
     | otherwise = -1
\end{code}
...
removeAll num (x:xs) = removeAll' num (x:xs) []
     where
          ...
          removeAll' num (x:xs) list 
               | num == x = removeAll' num xs list
               | otherwise = removeAll' num xs list ++ [x] 
               

--- List comprehensions
removeAllLC :: Int -> [Int] -> [Int]
removeAllLC num xs = [x | x <- xs, x /= num]
\end{code}
