...
\begin{code}
area :: Shape -> Float
area (Rectangle _ b h) = b * h
area (Circle _ r) = pi * (r^2)
area (Triangle a b c) = (dist(pMedio (a,b), c) * dist(a,b))/2

pMedio :: (Point, Point) -> Point
pMedio ((Point x1 y1), (Point x2 y2)) = (Point x11 y11)
     where 
          x11 = (x1+x2)/2
          y11 = (y1+y2)/2
\end{code}
...
\begin{code}
calculaFolhas :: IntTree -> Int
calculaFolhas ILeaf = 1
calculaFolhas (INode x l r) = calculaFolhas' (INode x l r) 0
    where
        calculaFolhas' ILeaf count = (count + 1)
        calculaFolhas' (INode x l r) count = (calculaFolhas' l count) + (calculaFolhas' r count)

calculaElementos :: IntTree -> Int
calculaElementos ILeaf = 0
calculaElementos (INode x l r) = (1 + (calculaElementos l) + (calculaElementos r))
\end{code}
...
intListToTree list = intListToTree' list ILeaf
  where 
    intListToTree' INil tree = tree
    intListToTree' (ICons x xs) tree = intListToTree' xs (insertIntTree x tree)

insertIntTree :: Int -> IntTree -> IntTree
insertIntTree x ILeaf = INode x ILeaf ILeaf
insertIntTree x (INode y l r) = case compare x y of
  LT -> INode y (insertIntTree x l) r
  EQ -> INode y l r
  GT -> INode y l (insertIntTree x r)
\end{code}
...
querOferta :: [Client] -> [Client]
querOferta list = [x | x <- list, (offers x) == True]
\end{code}