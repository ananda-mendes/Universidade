...
void lerInterrupcoes(Instrucao** instru) {
	FILE* arquivo = fopen ("interrupcoes.txt", "r");
	for(int i = 0; i < 30; i++){
		fscanf(arquivo, "%d",  &instru[i]->opcode);
		fscanf(arquivo, "%d",  &instru[i]->add1->endBloco);
		fscanf(arquivo, "%d",  &instru[i]->add1->endPalavra);
		fscanf(arquivo, "%d",  &instru[i]->add2->endBloco);
		fscanf(arquivo, "%d",  &instru[i]->add2->endPalavra);
		fscanf (arquivo, "%d", &instru[i]->add3->endBloco);
		fscanf (arquivo, "%d", &instru[i]->add3->endPalavra);
	}
	fclose(arquivo);	
}
